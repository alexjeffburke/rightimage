...
function arrangeImageTransformsAndApply(contentType, imageOptions) {
    ...
    const pipelineOptions = { type };
    for (const optionName of supportedPipelineOptions) {
        const optionValue = imageOptions[optionName];
        if (typeof optionValue === "undefined") continue;
        pipelineOptions[optionName] = optionValue;
        delete imageOptions[optionName];
    }
